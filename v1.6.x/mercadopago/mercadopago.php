...
class MercadoPago extends PaymentModule
{
    ...
    {
        UtilMercadoPago::logMensagem("Entro en hookPayment", MPApi::ERROR);
        if (!$this->active) {
            return;
        }
        ...
        if ($this->hasCredential()) {
            ...
            if (Configuration::get('MERCADOPAGO_STANDARD_ACTIVE') == 'true') {
                ...
                UtilMercadoPago::logMensagem($messageLog, MPApi::ERROR);
    ...
    private function getDimensions($products)
    {
        ...
        if (!($height > 0 && $length > 0 && $width > 0 && $weight > 0)) {
            ...
            UtilMercadoPago::logMensagem('MercadoPago :: getDimensions = '.$error, MPApi::INFO);
    ...
    public function createStandardCheckoutPreference()
    {
        ...
        if (Configuration::get('MERCADOPAGO_LOG') == 'true') {
            UtilMercadoPago::logMensagem("=====preferences 1 =====".Tools::jsonEncode($preferences), MPApi::INFO);
        }
    ...
    public function listenIPN($checkout, $topic, $id)
    {
        ...
        if ($checkout == 'standard' && $topic == 'merchant_order' && $id > 0) {
            ...
            foreach ($payments as $payment) {
                ...
                UtilMercadoPago::logMensagem('MercadoPago::listenIPN()::payment_info. ID: '.$payment['id'].', $status = '.$payment_info['status'], $id);
            ...
            if ($merchant_order_info['total_amount'] == $transaction_amounts) {
                ...
                UtilMercadoPago::logMensagem('MercadoPago::listenIPN()::transaction_amounts diferentes.: $merchant_order_info["total_amount"]: '.$merchant_order_info['total_amount'] .'!= $transaction_amounts: '.$transaction_amounts, $id);
            }
            // check the module
        ...
        }  elseif ($checkout == 'standard' && $topic == 'payment' && $id > 0) { 
            ...
            if (!isset($result['error'])) {
                ...
                UtilMercadoPago::logMensagem('MercadoPago::listenIPN()::Payment id_order: '.$id_order, $id);  
                
                // Como es un pago "payment" chequeo que ya existe el pedido si no no hago nada.
                if ($id_order > 0) {
                    ...
                    $new_order_status = $order_status[$payment_statuses[0]];
                    $new_id_order_status = Configuration::get($new_order_status);

                    // Entro solo si el pedido no tiene el estado nuevo. Si ya lo tiene no hago nada.
                    // Esto es para evitar doble pago completado, pendiente, etc.
                    if (!self::existsStatusInOrder($new_id_order_status, $id_order)) {

                        UtilMercadoPago::logMensagem('MercadoPago::listenIPN()::Payment id_order: '.$id_order.' ingresó con el estado: "' . $payment_statuses[0].'"', $id);               
                        ...
                        UtilMercadoPago::logMensagem('MercadoPago::listenIPN()::Payment id_order: '.$id_order.' ya existe el estado: "' . $payment_statuses[0] . '"', $id);
                    }
                }
            }
    ...
    public static function existsStatusInOrder($id_order_state, $id_order) {
        $count = Db::getInstance()->getValue(
            'SELECT count(`id_order_state`) FROM '._DB_PREFIX_.'order_history
        ...
        if (count($payment_statuses) == 1 ||
            ...
            if ($order_status) {
                ...
                if (is_numeric($id_order) && $id_order > 0) {
                    ...
                    if (self::checkStateExist($id_order, Configuration::get($order_status))) {
                        
                        if (Configuration::get('MERCADOPAGO_LOG') == 'true') {
                            UtilMercadoPago::logMensagem('MercadoPago::updateOrder() - Ya existe el estado '.$payment_status.' en el pedido ' . $id_order, 1);
                        }
                        return;
                    }
                }
                ...
                if ($id_order == false && ($payment_status == 'in_process' || $payment_status == 'approved' ||
                    ...
                    if (Configuration::get('MERCADOPAGO_LOG') == 'true') {
                        UtilMercadoPago::logMensagem('MercadoPago::updateOrder()::$id_order == '.$order->id.' && current_state = '.$order->current_state.' && order_status = ' . Configuration::get($order_status), 1);
                    }
    ...
    public function updateOrderHistory($id_order, $status, $mail = true)
    {
        ...
        if (Configuration::get('MERCADOPAGO_LOG') == 'true') {
            UtilMercadoPago::logMensagem('MercadoPago::updateOrderHistory()::checkStateExist('. $id_order . ', '. $status . ') = ' . ($existStates ? 'true' : 'false'), 1);
        }
    ...
    private function calculateList($postcode)
    {
            ...
            for ($qty = 0; $qty < $product['quantity']; ++$qty) {
                if ($product['width'] == 0) {
                    if (Configuration::get('MERCADOPAGO_LOG') == 'true') {
                        ...
                        UtilMercadoPago::logMensagem("=====dimensions=====".
                        $error, MPApi::ERROR);
                    }
        ...
        if (!($height > 0 && $length > 0 && $width > 0 && $weight > 0)) {
            $error = 'Invalid dimensions cart [height,length, width, weight].';
            if (Configuration::get('MERCADOPAGO_LOG') == 'true') {
                UtilMercadoPago::logMensagem("=====dimensions=====".$error, MPApi::ERROR);
            }
    ...
    private function calculate($params, $id_carrier)
    {
        ...
        if (!($height > 0 && $length > 0 && $width > 0 && $weight > 0)) {
            $error = 'Invalid dimensions cart [height, length, width, weight]';
            UtilMercadoPago::logMensagem("=====calculate=====".$error, MPApi::ERROR);
           // throw new Exception($error);
        }
    ...
    public function setSettings()
    {
        if ($this->hasCredential()) {
            ...
            try {
                ...
                if (Configuration::get('MERCADOPAGO_LOG') == 'true') {
                    UtilMercadoPago::logMensagem("=====settings=====".$e->getMessage(), MPApi::ERROR);
                }
            }
        }
    }
